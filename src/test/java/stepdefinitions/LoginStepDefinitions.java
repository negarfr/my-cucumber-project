...
//==> 1way. GET DATA AS LIST OF STRING
   List<String> customerData = credentials.row(1); //row index start at 0 and row() return List, so we put in list
     //credential has a data is coming from feature file like username and password
...
//        System.out.println(credentials.row(1).get(1));//row 2nd  and 2nd column

        loginPage.username.sendKeys(customerData.get(0));
        loginPage.password.sendKeys(customerData.get(1));
        loginPage.loginButton.click();

 //==> 2nd way. GET DATA AS <LIST<MAP<STRING,STRING>>
//
